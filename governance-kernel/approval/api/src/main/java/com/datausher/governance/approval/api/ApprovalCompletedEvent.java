...
public record ApprovalCompletedEvent(
    ...
    public ApprovalCompletedEvent {
        ...
        if (approvalRequest.status() == ApprovalRequestStatus.PENDING) {
            throw new IllegalArgumentException("approvalRequest must be terminal");
        }
    }
    ...
    public String eventType() {
        return switch (approvalRequest.status()) {
            case APPROVED -> ApprovalEvents.APPROVED;
            case REJECTED -> ApprovalEvents.REJECTED;
            case CANCELLED -> ApprovalEvents.CANCELLED;
            case PENDING -> throw new IllegalStateException("pending approval cannot be completed");
        };
    }
